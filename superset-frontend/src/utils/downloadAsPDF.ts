...
export default function downloadAsPDF(
  ...
  return (event: SyntheticEvent) => {
    ...
    // const width = (elementToPrint as any).offsetWidth;
    // const height = (elementToPrint as any).offsetHeight;
    ...
    return domToImage
      ...
      .then(dataUrl => {
        // computing image properties
        const imgProps = pdf.getImageProperties(dataUrl);
        const pdfWidth = pdf.internal.pageSize.getWidth();
        const pdfHeight = (imgProps.height * pdfWidth) / imgProps.width;

        // creating PDF from image dataUrl
        const fileName = `${generateFileStem(description)}.pdf`;
        const image = new Image();
        image.src = dataUrl;
        pdf.addImage(image, 'JPEG', 0, 0, pdfWidth, pdfHeight, 'fileName');
        pdf.output('datauri');
        pdf.save(fileName);
      })
      .catch(e => {
